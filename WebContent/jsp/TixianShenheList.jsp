    ...
    <table class="table table-hover text-center">
      ...
      <c:forEach items="${payrecordList}" var="d">
        <tr>
          ...
          <td>
          	<c:if test="${d.isCash==0 }">未审核</c:if>
	    	<c:if test="${d.isCash==1 }">已审核</c:if>
	    	<c:if test="${d.isCash==2 }">审核未通过</c:if>
          </td>
          <td>
          <div class="button-group"> 
          <a class="button border-main" href="${pageContext.request.contextPath }/payrecordServlet?action=shenhe&maiid=${d.payrecordid }"><span class="icon-edit">
          </span> 审核</a> 
          </div>
          </td>
        </tr>
